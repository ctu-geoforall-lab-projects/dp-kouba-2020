...
%% ML: poznamku pod carou bych vynechal a rovnou napsal "do CSV souboru a vektorovych vrstev do projektu QGIS".
...
%% ML: spise tam byly nedostatky, ktere bylo nutne pred zverejnenim pluginu vyresit
Avšak pro uvedení do \uv{ostrého} provozu musel být plugin více uživatelsky přívětivější.
Proto byl veškerý proces přesunut na pozadí, aby celý QGIS program \uv{nezamrzal} a
...
%% ML: prvni vetu prepiste, pusobi znacne kostrbate, projdete cely odstavec
...
do \uv{čístého} Python skriptu pomocí QGIS nástrojů i přes to, že lze z Grafického modeláře exportovat skript. 
...
dataset PID\_GTFS.
%% ML: slovo "dobrovolne" bych vynechal
...
%% ML: retezec (prekontrolujte v dalsich mistech prace)
...
%% ML: sjednodte v cele praci termin "Voronoi diagram"
