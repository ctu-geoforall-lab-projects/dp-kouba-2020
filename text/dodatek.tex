...
pluginu \textit{GTFS Loader} ve službě GitHub, je potřeba stáhnout komprimovaný ZIP soubor. 
To se provede tak, že se rozklikne tlačítko \textit{Code} a následně se vybere možnost
\textit{Download ZIP}.  

Pro načtení ZIP souboru v softwaru QGIS je potřeba otevřít stejné dialogové okno,
jako při instalaci pomocí QGIS repozitáře, ale následně se vybere možnost \textit{Install from ZIP}. 
Do uvedeného pole se zadá celá cesta ke staženému ZIP souboru a poté se stiskne tlačítko
...
Do pole s popiskem \textit{Select File} se musí vyplnit kompletní cesta k GTFS ZIP souboru (
pro vytváření pásem pouze k PID\_GTFS ZIP souboru). Dále stačí stisknout tlačítko \textit{Load}.
